...
-- Fonction: Vérifier certificats CEV expirants
CREATE OR REPLACE FUNCTION check_expiring_cev_certificates()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  FOR v_cev IN
    ...
    FROM cev_requests
    WHERE status = 'issued'
    AND cev_expiry_date IS NOT NULL
    AND cev_expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days'
  LOOP
    -- Créer notifications selon jours restants
    -- (sera implémenté dans notificationService)
    NULL;
  END LOOP;
END;
$$;
